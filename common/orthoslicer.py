...
class OrthoSlicer3D(object):
    ...
    def __init__(self, data, axes=None, aspect_ratio=np.array((1,1,1))):
        ...
        kw = dict(vmin=data.min(), vmax=data.max(), picker=True,
                aspect='auto')
        im1 = ax1.imshow(data[...,0], plt.cm.bone, interpolation='none', **kw)
        im2 = ax2.imshow(data[:,0,:], plt.cm.bone, interpolation='none', **kw)
        im3 = ax3.imshow(data[0,...], plt.cm.bone, interpolation='none', **kw)
