...
file_names = ['k_space_real', 'k_space_real_gt', 'mask']
...
if to_show:
    ...
    for i in range(0, mini_batch_size):
        ...
        imshow(batch["k_space_real_gt"][i,:,:], ax=ax[0][0], fig=fig)

        ax[0][1].set_title('k-space - subsampeld')
        imshow(batch["k_space_real"][i,:,:], ax=ax[0][1], fig=fig)

        ax[1][0].set_title('k-space - interpolation')
        imshow(x_upscaled[i,:,:].transpose(), ax=ax[1][0], fig=fig)

        ax[1][1].set_title('k-space - predicted')
        imshow(y_pred[i,:,:].transpose(), ax=ax[1][1], fig=fig)
