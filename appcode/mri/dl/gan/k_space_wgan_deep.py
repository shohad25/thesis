...
class KSpaceSuperResolutionWGAN(BasicModel):
    ...
    def __G__(self):
        ...
        self.conv_6_bn = ops.batch_norm(self.conv_6, self.train_phase, decay=0.98, name="G_bn6")
        self.relu_6 = tf.nn.relu(self.conv_6_bn)

        out_dim = 8
        self.conv_7 = ops.conv2d(self.relu_6, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_conv_7")
        self.conv_7_bn = ops.batch_norm(self.conv_7, self.train_phase, decay=0.98, name="G_bn7")
        self.relu_7 = tf.nn.relu(self.conv_7_bn)

        out_dim = 2
        self.conv_8 = ops.conv2d(self.relu_7, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_conv_8")

        predict = {}
        predict['real'] = tf.reshape(self.conv_8[:,0,:,:], [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]], name='G_predict_real')
        predict['imag'] = tf.reshape(self.conv_8[:,1,:,:], [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]], name='G_predict_imag')
    ...
    def __D__(self, input_d, input_type):
        ...
        out_dim = 1
        self.affine_1_d = ops.linear(tf.contrib.layers.flatten(self.relu_5_d), output_size=out_dim, scope="D_affine_1")
