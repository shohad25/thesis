...
class KSpaceSuperResolutionWGAN(BasicModel):
    ...
    def __G__(self):
        ...
        out_dim = 128   # 32x32 => 32x32
        conv4 = ops.conv_conv_pool(pool3, n_filters=[out_dim, out_dim], activation=tf.nn.relu,
                                          training=self.train_phase, name='G_block_4', pool=False)

        # concat (32x2)x(32x2) with 64x64
        up7 = ops.upsample_concat(inputA=conv4, input_B=conv3, name='G_block_7')
        out_dim = 32
