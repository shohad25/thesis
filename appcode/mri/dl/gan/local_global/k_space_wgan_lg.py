...
class KSpaceSuperResolutionWGAN(BasicModel):
    ...
    def build(self):
        ...
        with tf.name_scope('D_'):
            ...
            reconstructed_image_reference = self.get_reconstructed_image(real=self.input_d['real'],
                                                                         imag=self.input_d['imag'], name='Both_gt')

            reconstructed_image_reference = tf.expand_dims(input=tf.complex(real=reconstructed_image_reference[:,0,:,:],
    ...
    def __G2__(self, g2_input):
        ...
        tf.summary.image('G2_reconstructed_input' + 'Fake_from_G', tf.transpose(reconstructed_image, (0, 2, 3, 1)), collections='G',
                         max_outputs=4)
        ...
        self.conv_3_g2_residual = self.conv_3_g2 + reconstructed_image

	self.g2_out = self.conv_3_g2_residual
