...
class KSpaceSuperResolutionWGAN(BasicModel):
    ...
    def __G__(self):
        ...
        relu_1 = tf.nn.relu(conv_1, name='G_relu1')

        # Pool to 128x128
        conv_2 = ops.conv2d(relu_1, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_conv_2")
        pool_2 = tf.layers.max_pooling2d(conv_2, pool_size=[2, 2], strides=2, padding='same',
        ...
        relu_3 = tf.nn.relu(conv_3, name='G_relu3')

        # Pool to 64x64
        conv_4 = ops.conv2d(relu_3, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_conv_4")
        pool_4 = tf.layers.max_pooling2d(conv_4, pool_size=[2, 2], strides=2, padding='same',
        ...
        relu_5 = tf.nn.relu(conv_5, name='G_relu5')

        # Add noise GAN
        # print "Noise level: (-0.05,0.05)"
        # minval = -0.05
        # maxval = 0.05
        # noise = tf.random_uniform(shape=tf.shape(relu_5), minval=minval, maxval=maxval, dtype=tf.float32, seed=None,
        #                           name='noise')
        # relu_5 += noise
        ...
        relu_8 = tf.nn.relu(conv_8, name='G_relu8')
