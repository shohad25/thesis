...
class KSpaceSuperResolutionGAN(BasicModel):
    ...
    def build(self):
        ...
        with tf.name_scope('D_'):
            ...
            self.predict_d, self.predict_d_for_g = tf.split(value=self.predict, num_split=2, split_dim=0)
            self.predict_d_logits, self.predict_d_logits_for_g = tf.split(value=self.predict_logits, num_split=2, split_dim=0)
    ...
    def __G__(self):
        ...
        out_dim = 16
        out_shape = [self.batch_size, self.dims_out[0], self.dims_out[1], out_dim]
        self.conv_1, reg_1 = ops.conv2d_transpose(x_input_stack, output_shape=out_shape, k_h=3, k_w=3, d_h=2, d_w=1, name="G_conv_1")
        ...
        #     out_dim = 16
        #     out_shape = [self.batch_size, self.dims_out[0], self.dims_out[1], out_dim]
        #     self.conv_11, reg_11 = ops.conv2d_transpose(x_imag, output_shape=out_shape, k_h=3, k_w=3, d_h=2, d_w=1, name="G_conv_11")
        #     # out_dim = 8
    ...
    def __D__(self, input_d, input_type):
        ...
        input_real = tf.concat(concat_dim=0, values=[input_d[0]['real'], input_d[1]['real']])
        input_imag = tf.concat(concat_dim=0, values=[input_d[0]['imag'], input_d[1]['imag']])
        ...
        org, fake = tf.split(value=input_to_discriminator, num_split=2, split_dim=0)
    ...
    def get_reconstructed_image(self, real, imag, name=None):
        ...
        # Shifting
        top, bottom = tf.split(value=rec_image, num_split=2, split_dim=1)
        top_left, top_right = tf.split(value=top, num_split=2, split_dim=2)
        bottom_left, bottom_right = tf.split(value=bottom, num_split=2, split_dim=2)

        top_shift = tf.concat(concat_dim=2, values=[bottom_right, bottom_left])
        bottom_shift = tf.concat(concat_dim=2, values=[top_right, top_left])
        shifted_image = tf.concat(concat_dim=1, values=[top_shift, bottom_shift])
