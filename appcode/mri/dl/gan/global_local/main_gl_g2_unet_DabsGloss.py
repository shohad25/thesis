...
from appcode.mri.dl.gan.global_local.k_space_gl_g2_unet_DabsGloss import KSpaceSuperResolutionGLWGAN
...
def run_evaluation(sess, feed, net, step, writer, tt):
    ...
    print('%s:  Time: %s , Loss at step %s: D: %s, D_no_reg: %s, G: %s, G_no_reg: %s, L2: %s' % (tt, datetime.datetime.now(), step, 0, 0, loss_g, g_loss_no_reg, l2_norm))
    logfile.writelines('%s: Time: %s , Accuracy at step %s: D: %s, G: %s, L2: %s\n' % (tt, datetime.datetime.now(), step, 0, loss_g, l2_norm))
    logfile.flush()
...
def load_graph():
    ...
    network = KSpaceSuperResolutionGLWGAN(input=None, labels=y_input, dims_in=DIMS_IN,
...
def evaluate_checkpoint(tt='test', checkpoint=None, output_file=None, output_file_interp=None):
    ...
    while data_set_tt.epoch == 0:
            ...
            if feed is not None:
                feed[net.adv_loss_w] = gen_loss_adversarial
                predict, result = sess.run([net.predict_g2, net.evaluation], feed_dict=feed)
...
def main(args):
    ...
    if args.mode == 'train' or args.mode == 'resume':
        ...
        model_file = inspect.getfile(KSpaceSuperResolutionGLWGAN)
