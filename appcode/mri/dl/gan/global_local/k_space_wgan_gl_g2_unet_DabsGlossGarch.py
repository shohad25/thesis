...
class KSpaceSuperResolutionGLWGAN(BasicModel):
    ...
    def build(self):
        ...
        with tf.name_scope('D_'):
            ...
            self.reconstructed_image_reference = tf.complex(real=self.reconstructed_image_reference[:,0,:,:],
                                                            imag=self.reconstructed_image_reference[:,1,:,:])
            self.reconstructed_image_reference = tf.abs(tf.expand_dims(input=self.reconstructed_image_reference, axis=1))

            self.predict, self.predict_logits = self.__D__([self.reconstructed_image_reference, self.predict_g2['rec']])
    ...
    def __G2__(self):
        ...
        conv1, pool1 = ops.conv_conv_pool(reconstructed_image_to_show, n_filters=[out_dim, out_dim], activation=tf.nn.relu,
        ...
        predict = {}
        predict['rec'] = conv_last

        # residual
        with tf.name_scope("final_predict_2"):
            predict['rec'] = tf.add(predict['rec'], reconstructed_image_to_show, name='rec')

        tf.add_to_collection("predict", predict['rec'])

        # Dump prediction out
        if self.FLAGS.dump_debug:
            g2_image_to_show = predict['rec']
    ...
    def __D__(self, input_d):
        ...
        rec_org = input_to_discriminator[0]
        rec_fake = input_to_discriminator[1]
    ...
    def __loss__(self):
        ...
        # Context loss L2
        predict_image = self.predict_g2['rec']
