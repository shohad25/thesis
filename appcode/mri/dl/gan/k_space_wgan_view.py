...
class KSpaceSuperResolutionWGAN(BasicModel):
    ...
    def __init__(self, input=None, labels=None, dims_in=None, dims_out=None, FLAGS=None, train_phase=None, adv_loss_w=None):
        ...
        # HACK
        self.input = {}
        self.input['real'] = tf.zeros((5,1, 256,256))
        self.input['imag'] = tf.zeros((5,1, 256, 256))
        self.input['mask'] = tf.zeros((5,1, 256, 256))
        self.labels = self.input

        self.dims_in = [1,256,256]
        self.dims_out = [1, 256, 256]
        self.input_d = self.labels
        # HACK
...
if __name__ == '__main__':
    ...
    network = KSpaceSuperResolutionWGAN(input=None, labels=None, dims_in=DIMS_IN,
                                      dims_out=DIMS_OUT, FLAGS=None, train_phase=False, adv_loss_w=1.0)
    network.build()
    sess = tf.Session()
    tf.train.write_graph(sess.graph_def, '/tmp/train/', 'graph.pbtxt', True)
    ...
    writer['train']['D'] = tf.summary.FileWriter(os.path.join('/tmp/train/', 'train', 'D'), sess.graph)
    writer['train']['G'] = tf.summary.FileWriter(os.path.join('/tmp/train/', 'train', 'G'), sess.graph)

    r_g, r_d = sess.run([m_op_g, m_op_d])
    writer['G'].add_summary(r_g, 0)
    writer['D'].add_summary(r_d, 0)