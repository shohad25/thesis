...
for i in range(3,100):
    ...
    # ax[3].imshow(np.log(1+np.abs(k_space_real_gt**2 + k_space_imag_gt**2)), interpolation="none", cmap="gray")
    # ax[3].axis('off')
    ...
    fig.savefig('/tmp/example_samplingMask4.png', bbox_inches=extent, format=IMG_FORMAT, dpi=DPI)
    # extent = ax[3].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    # fig.savefig('/tmp/example_kSpace.png', bbox_inches=extent, format=IMG_FORMAT, dpi=DPI)
