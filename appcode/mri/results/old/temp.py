...
from appcode.mri.k_space.utils import get_image_from_kspace, interpolated_missing_samples
    ...
    while data_set_tt.epoch == 0:
        ...
        for i in range(0, data["k_space_real_gt"].shape[0]):
            for move in [-4, 0 , 4]:
                ...
                mask = get_random_mask(w=256, h=256, factor=5, start_line=movi_x, keep_center=0.1)
                ...
                # mask = np.zeros_like(k_space_real_gt)
                # mask[120:140, :] = 1
                mask = get_random_mask(w=256, h=256, factor=2, start_line=0, keep_center=0.0)
                reduction = np.sum(mask) / float(mask.ravel().shape[0])
                print (reduction)
                k_space_real_gt = data["k_space_real_gt"][i,:,:] * mask
                k_space_imag_gt = data["k_space_imag_gt"][i,:,:] * mask
                ...
                k_space_amp_gt = np.log(1+np.sqrt(k_space_real_gt**2 + k_space_imag_gt**2))
                org_image = get_image_from_kspace(k_space_real_gt,k_space_imag_gt)
                ax[0][3].set_title('Filter')
                ax[0][3].imshow(org_image, interpolation="none", cmap="gray")

                ax[1][3].set_title('K-space')
                ax[1][3].imshow(k_space_amp_gt, interpolation="none", cmap="gray")

                ax[2][3].set_title('Sampling Mask')
                ax[2][3].imshow(mask, interpolation="none", cmap="gray")
